...
def calcular():
    ...
    if int(x.get())+int(y.get())>int(z.get()) and int(x.get())+int(z.get())>int(y.get()) and int(z.get())+int(y.get())>int(x.get()):
        t_resultados.insert(INSERT, "Si es un triangulo ")
    else:
        t_resultados.insert(INSERT, "No es un triangulo ")
...
lb_logo.place(x=10,y=30)


# etiqueta para el titulo de la app
titulo = Label(frame_entrada, text= "verificar si es triangulo")
titulo.config(bg="white", fg="blue", font=("Arial",16))
titulo.place(x=240,y=10, width=230, height=30)

# etiqueta para x
lb_x = Label(frame_entrada, text = "Lado a = ")
...
# etiqueta para y
lb_y = Label(frame_entrada, text = "Lado b= ")
